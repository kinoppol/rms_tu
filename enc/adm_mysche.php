...
<tr>
<td width="33%" align="left" colspan="2"><a href="';echo $_SERVER['PHP_SELF'] .'?m='.$p_month .'&y='.$p_year.'&p='.$_REQUEST[p].'&mod='.$_REQUEST[mod];;echo '" > < ก่อนหน้า</a></td>
<td width="33%" align="center" colspan="3">  <a href="';echo $_SERVER['PHP_SELF'] .'?m='.date(m) .'&y='.date(Y).'&p='.$_REQUEST[p].'&mod='.$_REQUEST[mod];;echo '" > วันนี้ </a></td>
<td width="33%" align="right" colspan="2"><a href="';echo $_SERVER['PHP_SELF'] .'?m='.$n_month .'&y='.$n_year.'&p='.$_REQUEST[p].'&mod='.$_REQUEST[mod];;echo '" >ถัดไป ></a>  </td>
</tr>
...
$font_color = "<a href='".$_SERVER['PHP_SELF'] .'?m='.$cur_m .'&y='.$cur_y.'&d='.sprintf('%02d',($i -$startday +1)).'&p='.$_REQUEST[p].'&mod='.$_REQUEST[mod]."' style='color:#336699;font-size:14pt'>";
$cell_color = '#BBDDFF';
}else{
$font_color = "<a href='".$_SERVER['PHP_SELF'] .'?m='.$cur_m .'&y='.$cur_y.'&d='.sprintf('%02d',($i -$startday +1)).'&p='.$_REQUEST[p].'&mod='.$_REQUEST[mod]."' style='color:#000000'>";
$cell_color = '#F0F0F0';
}
...
$sql_detail = "SELECT * FROM sch_people WHERE school_id= '$_SESSION[school_id]' $datesearch and people_id = '$_SESSION[userid]'  ";
...
$sql_detail = "SELECT * FROM sch_people WHERE school_id= '$_SESSION[school_id]' $datesearch and people_id = '$_SESSION[userid]' ORDER BY sch_detail_startdate ";
